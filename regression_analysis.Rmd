...
load(file = "environment_20200909.RData")
...
partial_models[["RE_perf_abs"]] <- list()
partial_models[["EE_perf_abs"]] <- list()
...
All correlations above 0.8 are problematic. We don't find any.

```{r}
faraway::vif(full_models$RE$`01_untransformed`)
```
VIF above 5 is critical and above 10 dangerous. P_density, S_GAV and S_potential have critical VIFs

## Math variable selection

```{r}
full_models[["RE_perf_abs"]][["03_selected"]] <- step(
  full_models[["RE_perf_abs"]][["01_untransformed"]], direction = "backward", trace = 0)
summary(full_models[["RE_perf_abs"]][["03_selected"]])

```
Results could be skewed due to collinearities, as all 3 critical IV are included.
...
```{r}
resplot(full_models[["RE_perf_abs"]][["03_selected"]])
```
...
```{r, warning=FALSE, echo=FALSE}
GGally::ggcorr(EE_both_effects$wide_no_na, nbreaks = 5, label = TRUE, size = 2, label_size = 3, legend.position = "bottom", legend.size = 10)
```
...
```
VIF above 5 is critical and above 10 dangerous. Multiple variable have critical VIFs. In particular S_potential. Needs to be fixed.

## Math variable selection

```{r}
full_models[["EE_perf_abs"]][["03_selected"]] <- step(
  full_models[["EE_perf_abs"]][["01_untransformed"]], direction = "backward", trace = 0)
summary(full_models[["EE_perf_abs"]][["03_selected"]])
...
```{r}
resplot(full_models[["EE_perf_abs"]][["03_selected"]])
```

Could be off, Scale-Location plot looks off.
...
I'm not sure that we can trust this model. The Tukey-Anscombe plot is skewed, probably because of a highly influential outlier (HR).
...
```{r}
faraway::vif(full_models$EE_amb$`01_untransformed`)
```

S_potential could be collinear with S_GDP
...
S_potential and S_GDP not in the final model, so collinearity issue not a problem here.
...
These assumption plots are not ideal, but no highly influential outliers.
...
print_table_of_models <- function(variable, title = ""){
  ...
  tab_model(
    ...
    dv.labels = c("Full model", "Selected model"),
