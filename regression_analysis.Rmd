...
```{r full model with transformation}

# formulas[["REN_perf_full"]] <- formula(RE_perf ~  
#     P_density + P_ambition + P_REN +
#     S_GDP + S_GAV + S_potential + S_effort + S_PPS + S_elec + S_centr)
# 
...
formulas[["REN_perf_full"]] <- formula(RE_perf ~  
    P_density + P_ambition + P_REN +
     S_GAV + S_potential + S_effort + S_PPS + S_elec  + S_centr + S_size_industry + S_size_services)
...
```{r correlation check, warning=FALSE, echo=FALSE}
...
full_models[["RE"]][["01_manual"]] <- lm(
  RE_perf ~  S_size_services, data = Ren_both_effects[["wide_no_na"]])

summary(full_models$RE$`01_manual`)
...
formulas[["EE_perf_full"]] <- formula(EE_perf ~  
    P_density + P_ambition + P_REN +
    S_GAV + S_potential + S_effort + S_PPS + S_elec + S_centr  + S_size_services + S_size_industry)

# GDP may be collinear
# formulas[["EE_perf_full"]] <- formula(EE_perf ~  
#     P_density + P_ambition + P_REN +
#     S_GDP + S_GAV + S_potential + S_effort + S_PPS + S_elec + S_centr  + S_size_industry + S_size_services)
...
```{r correlation check EE, warning=FALSE}
...
# P_ambition      -3.035e-01  1.232e-01  -2.463 0.033479 *  
# P_REN            2.705e-03  9.322e-04   2.901 0.015800 *  
# S_GAV            4.474e+00  1.502e+00   2.978 0.013867 *  
# S_effort        -5.731e+01  1.800e+01  -3.184 0.009746 ** 
# S_PPS           -2.624e-03  5.677e-04  -4.621 0.000949 ***
# S_size_services -1.063e+00  3.725e-01  -2.854 0.017129 *  
# S_size_industry -7.841e-01  2.639e-01  -2.971 0.014028 *  
# ---
  
  
full_models[["RE"]][["01_manual"]] <- lm(
  EE_perf ~  S_PPS, data = EE_both_effects[["wide_no_na"]])
...
We collected data for 28 countries for 3 dependent (DP), and X potentially explaning independent variables (IV); see table X. As some data was missing for some countries, we constructed multiple linear regression (MLR) models for 23 countreis to explain REN performance/ambition and 22 countries to explain EE performance/ambition. To avoid any collinearity, we analysed correlations between all independent variables (IV) and calculated variance inflation factors, but did not need to exclude any of the IV due to collinearity.

We constructed tested several models: 
- Full models include all of the IVs in Table X and the corresponding DV
-	Optimised models are constructed using a mathematical selection algorithm, namely a stepwise backward selection of variables which reduces a model’s AIC.
-	Policy models are only constructed using the policy group variables
-	Economic-context models are only constructed using the economic-context group variables

To check MLR assumptions, we analysed residual plots (Tukey-Anscombe, normal, scale-location and leverage plot). We used a random resampling approach to support our assumption analysis; i.e. we resampled new residuals to the existing fitted values, and hence were able to tell if any deviation from the optimal regression assumptions were systematic or random.

We use the 10% significance level, as also done by... TODO!

LANA: I did some more analyses, if necessary they can also be added, but for now I would not include them.

# Data

The data for the variables in Table X is a sample of all European countries. Hence, the sample is complete in terms of geographical coverage. However, due to missing values for certain variables, some countries could not be used in the MLR model construction. Also, the variables are often averaged values of certain years, and other samples may exist that are based on other years. Hence the main purpose of this statistical analysis is first to uncover which models could explain our DVs, and second, wheter these models are also valid if other samples were constructed.
